...
import { WalletResponseDto, SendPaymentDto, createApiResponse, createErrorResponse } from "../dtos/WalletDTO"
...
export class WalletController {
  ...
  async getWallet(req: Request, res: Response) {
    try {
      ...
      if (!userId) {
        return res.status(401).json(createErrorResponse("User ID not found in request"))
      }
  ...
  async getTransactions(req: Request, res: Response) {
    try {
      ...
      const result = await this.walletService.getTransactions(wallet.id, page, limit, sort)
  ...
  async sendPayment(req: Request, res: Response) {
    try {
      ...
      if (!errors.isEmpty()) {
        return res.status(400).json(createErrorResponse("Validation failed", "VALIDATION_ERROR", errors.array()))
      }
      ...
      if (validationErrors.length > 0) {
        return res
          .status(400)
          .json(createErrorResponse("DTO validation failed", "DTO_VALIDATION_ERROR", validationErrors))
      }
      ...
      const { destinationAddress, amount, assetCode, assetIssuer, memo } = sendPaymentDto
      ...
      res.status(201).json(createApiResponse(transaction, "Payment sent successfully"))
    } catch (error: any) {
      if (error.message.includes("insufficient")) {
        return res.status(400).json(createErrorResponse("Insufficient funds", "INSUFFICIENT_FUNDS"))
      }
      if (error.message.includes("Invalid")) {
        return res.status(400).json(createErrorResponse(error.message, "INVALID_INPUT"))
      }
      res.status(500).json(createErrorResponse(error.message))
    }
  }
  ...
  async activateWallet(req: Request, res: Response) {
    try {
      ...
      const activatedWallet = await this.walletService.activateWallet(wallet.id)
      res.json(createApiResponse(activatedWallet, "Wallet activated successfully"))
    } catch (error: any) {
      res.status(500).json(createErrorResponse(error.message))
    }
  }
  ...
  async verifyWallet(req: Request, res: Response) {
    try {
      ...
      res.json(createApiResponse(verifiedWallet, "Wallet verified successfully"))
    } catch (error: any) {
      res.status(500).json(createErrorResponse(error.message))
    }
  }
  ...
  async updateSettings(req: Request, res: Response) {
    try {
      ...
      const updatedWallet = await this.walletService.updateWalletSettings(wallet.id, req.body)
      res.json(createApiResponse(updatedWallet, "Wallet settings updated successfully"))
    } catch (error: any) {
      res.status(500).json(createErrorResponse(error.message))
    }
  }
  ...
  async estimateFee(req: Request, res: Response) {
    try {
      const operationCount = Number.parseInt(req.query.operations as string) || 1

      if (operationCount < 1 || operationCount > 100) {
        return res.status(400).json(createErrorResponse("Invalid operation count. Must be between 1 and 100"))
      }
      ...
      const estimatedFee = await this.walletService.estimateTransactionFee(operationCount)
