...
const mockedRateLimitMonitoringService = jest.mocked(RateLimitMonitoringService)
...
describe("intelligentRateLimiter", () => {
  ...
  beforeEach(() => {
    ...
    mockedRateLimitConfigService.getConfigForUser.mockResolvedValue(createMockRateLimitConfig(60))
    ...
    mockedRateLimitMonitoringService.logAdvancedRateLimitEvent.mockResolvedValue(undefined)
  })
  ...
  it("should activate burst mode and return burst limit if user is authenticated and limit exceeded", async () => {
    ...
    expect(mockResponse.setHeader).toHaveBeenCalledWith("X-RateLimit-Burst", "activated")
    expect(mockResponse.setHeader).toHaveBeenCalledWith("X-RateLimit-Burst-Duration", "30")
