...
describe("ReferralService", () => {
  ...
  describe("processReferralSignup", () => {
    ...
    it("should process a valid referral signup", async () => {
      const processedReferral = await referralService.processReferralSignup(referral.referralCode, testUser2.id)
    ...
    it("should create rewards when processing referral", async () => {
      await referralService.processReferralSignup(referral.referralCode, testUser2.id)

      const rewardRepo = AppDataSource.getRepository(ReferralReward)
      const rewards = await rewardRepo.find({ where: { referralId: referral.id } })

      expect(rewards).toHaveLength(2)

      const referrerReward = rewards.find((r) => r.type === RewardType.REFERRER)
    ...
    it("should throw error for invalid referral code", async () => {
      await expect(referralService.processReferralSignup("INVALID", testUser2.id)).rejects.toThrow(
        "Invalid referral code",
      )
    })

    it("should throw error for self-referral", async () => {
      await expect(referralService.processReferralSignup(referral.referralCode, testUser1.id)).rejects.toThrow(
    ...
    it("should throw error for already used referral code", async () => {
      ...
      await expect(referralService.processReferralSignup(referral.referralCode, testUser3.id)).rejects.toThrow(
  ...
  describe("processRewardPayment", () => {
    ...
    it("should process reward payment successfully", async () => {
      ...
      const processedReward = await referralService.processRewardPayment(reward.id, transactionHash)
    ...
    it("should throw error for non-existent reward", async () => {
      await expect(referralService.processRewardPayment(99999, "hash")).rejects.toThrow("Reward not found")
    })

    it("should throw error for already paid reward", async () => {
      await referralService.processRewardPayment(reward.id, "hash1")

      await expect(referralService.processRewardPayment(reward.id, "hash2")).rejects.toThrow(
        "Reward is not in pending status",
      )
    })
  })
  ...
  describe("validateReferralCode", () => {
    ...
    it("should validate unused referral code", async () => {
      const isValid = await referralService.validateReferralCode(referral.referralCode)
    ...
    it("should invalidate expired referral code", async () => {
      ...
      const isValid = await referralService.validateReferralCode(expiredReferral.referralCode)
      expect(isValid).toBe(false)
    })
  })
})
