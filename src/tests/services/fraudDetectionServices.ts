...
describe('FraudDetectionService - Rate Limit Integration', () => {
  ...
  beforeAll(() => {
    ...
    mockRateLimitHistoryRepo = AppDataSource.getRepository(RateLimitHistory) as jest.Mocked<Repository<RateLimitHistory>>;
    mockTransactionRepo = AppDataSource.getRepository(Transaction) as jest.Mocked<Repository<Transaction>>;
    mockMerchantConfigRepo = AppDataSource.getRepository(MerchantFraudConfig) as jest.Mocked<Repository<MerchantFraudConfig>>;
  });
  ...
  beforeEach(() => {
    ...
    mockTransactionRepo.createQueryBuilder().getRawOne.mockResolvedValue({ total: 0 });
    mockTransactionRepo.find.mockResolvedValue([]);
    (service as any).getAverageTransactionAmount = jest.fn().mockResolvedValue(100); // Mock internal helper
  });
  ...
  describe('checkRateLimitingPatterns', () => {
    it('should add score for excessive rate limiting', async () => {
      ...
      mockRateLimitHistoryRepo.createQueryBuilder().getRawOne.mockResolvedValue({ endpointCount: 0 });
    ...
    it('should add score for rapid endpoint switching', async () => {
      mockRateLimitHistoryRepo.count.mockResolvedValue(0);
      mockRateLimitHistoryRepo.createQueryBuilder().getRawOne.mockResolvedValue({ endpointCount: 11 }); // Rapid endpoint switching
      mockRateLimitHistoryRepo.count.mockResolvedValue(0);
  ...
  describe('checkTransactionWithRateLimit', () => {
    ...
    it('should blacklist user if rate limit risk score is high', async () => {
      jest.spyOn(service, 'checkTransaction').mockResolvedValueOnce({
        riskScore: 0, riskLevel: RiskLevel.LOW, shouldBlock: false, rulesTriggered: [], requiresReview: false,
      });
      jest.spyOn(service, 'checkRateLimitingPatterns').mockResolvedValueOnce({
  ...
  describe('getRateLimitFraudStats', () => {
    it('should return aggregated rate limit fraud statistics', async () => {
      const mockHistoryData = [
        { wasThrottled: true, wasBurst: false, ip: '1.1.1.1', userId: 'userA' },
        { wasThrottled: true, wasBurst: false, ip: '1.1.1.1', userId: 'userA' },
        { wasThrottled: true, wasBurst: false, ip: '1.1.1.1', userId: 'userA' },
        { wasThrottled: true, wasBurst: false, ip: '1.1.1.1', userId: 'userA' },
        { wasThrottled: true, wasBurst: false, ip: '1.1.1.1', userId: 'userA' },
        { wasThrottled: true, wasBurst: false, ip: '1.1.1.1', userId: 'userA' }, // 6 throttled for userA
        { wasThrottled: true, wasBurst: false, ip: '2.2.2.2', userId: 'userB' },
        { wasThrottled: true, wasBurst: false, ip: '2.2.2.2', userId: 'userB' },
        { wasThrottled: true, wasBurst: false, ip: '2.2.2.2', userId: 'userB' },
        { wasThrottled: true, wasBurst: false, ip: '2.2.2.2', userId: 'userB' },
        { wasThrottled: true, wasBurst: false, ip: '2.2.2.2', userId: 'userB' },
        ...
        { wasThrottled: true, wasBurst: false, ip: '2.2.2.2', userId: 'userB' },
        { wasThrottled: true, wasBurst: false, ip: '2.2.2.2', userId: 'userB' },
        { wasThrottled: true, wasBurst: false, ip: '2.2.2.2', userId: 'userB' }, // 11 throttled for IP 2.2.2.2
        { wasThrottled: false, wasBurst: true, ip: '3.3.3.3', userId: 'userC' },
        { wasThrottled: false, wasBurst: true, ip: '3.3.3.3', userId: 'userC' },
      ].map(e => ({ ...e, timestamp: new Date(), merchantId: 'merchant123' }));

      mockRateLimitHistoryRepo.createQueryBuilder().getMany.mockResolvedValue(mockHistoryData as any);
      mockRateLimitHistoryRepo.createQueryBuilder().getRawMany
        .mockResolvedValueOnce([{ ip: '2.2.2.2', count: '11' }]) // suspiciousIPs
        .mockResolvedValueOnce([{ userId: 'userA', count: '6' }]); // suspiciousUsers
