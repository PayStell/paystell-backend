...
describe("ReferralController", () => {
  ...
  beforeEach(async () => {
    ...
    // Get auth tokens
    const loginResponse1 = await authService.login("test1@example.com", "password123")
    const loginResponse2 = await authService.login("test2@example.com", "password123")
  ...
  describe("POST /api/referrals", () => {
    it("should create a referral successfully", async () => {
      const response = await request(app).post("/api/referrals").set("Authorization", `Bearer ${authToken1}`).send({})
  ...
  describe("GET /api/referrals", () => {
    ...
    it("should get user referrals", async () => {
      const response = await request(app).get("/api/referrals").set("Authorization", `Bearer ${authToken1}`)
  ...
  describe("GET /api/referrals/stats", () => {
    ...
    it("should get user referral stats", async () => {
      const response = await request(app).get("/api/referrals/stats").set("Authorization", `Bearer ${authToken1}`)
  ...
  describe("GET /api/referrals/validate/:code", () => {
    ...
    it("should validate valid referral code", async () => {
      const response = await request(app).get(`/api/referrals/validate/${referralCode}`)
    ...
    it("should invalidate invalid referral code", async () => {
      const response = await request(app).get("/api/referrals/validate/INVALID")
