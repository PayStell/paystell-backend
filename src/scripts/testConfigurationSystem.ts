...
import { ConfigurationType, ConfigurationCategory } from "../entities/Configuration";
...
async function testConfigurationSystem() {
  try {
    ...
    const testConfig = await configurationService.setConfig("TEST_CONFIG", "test_value", {
    ...
    const typeTests = [
      ...
      { key: "JSON_CONFIG", value: { key: "value", nested: { data: "test" } }, type: ConfigurationType.JSON },
    ];
    ...
    for (const test of typeTests) {
      ...
      logger.info(`✅ ${test.key}: ${JSON.stringify(retrieved)} (${test.type})`);
    }

    // Test 3: Encrypted Configuration
    logger.info("\n📝 Test 3: Encrypted Configuration");
    
    await configurationService.setConfig("SECRET_CONFIG", "super_secret_value", {
      type: ConfigurationType.STRING,
    ...
    const encryptedValue = await configurationService.getConfig("SECRET_CONFIG");
    ...
    const globalEvaluation = await configurationService.evaluateFeatureFlag("test_global_flag");
    logger.info(`✅ Global flag evaluation: ${globalEvaluation.isEnabled} - ${globalEvaluation.reason}`);

    const targetedEvaluation1 = await configurationService.evaluateFeatureFlag("test_targeted_flag", {
      userId: "user1",
    });
    logger.info(`✅ Targeted flag evaluation (user1): ${targetedEvaluation1.isEnabled} - ${targetedEvaluation1.reason}`);

    const targetedEvaluation2 = await configurationService.evaluateFeatureFlag("test_targeted_flag", {
      userId: "user3",
    });
    logger.info(`✅ Targeted flag evaluation (user3): ${targetedEvaluation2.isEnabled} - ${targetedEvaluation2.reason}`);
    ...
    logger.info(`✅ Encrypted configurations: ${allConfigs.filter(c => c.isEncrypted).length}`);
    logger.info(`✅ Required configurations: ${allConfigs.filter(c => c.isRequired).length}`);
    ...
    logger.info(`✅ Cache performance test: ${endTime - startTime}ms for 5 requests`);
    ...
    await configurationService.setConfig("ENV_SPECIFIC_CONFIG", `value_for_${currentEnv}`, {
      type: ConfigurationType.STRING,
      category: ConfigurationCategory.GENERAL,
    ...
    const envConfig = await configurationService.getConfig("ENV_SPECIFIC_CONFIG");
