...
class RateLimitMonitoringService {
  ...
  constructor() {
    ...
    if (AppDataSource.isInitialized) {
      ...
      AppDataSource.initialize()
        ...
        .catch((error) => {
          logger.error(
            "Failed to initialize rate limit history repository:",
            error,
          );
        });
    }
  ...
  public async logAdvancedRateLimitEvent(
    event: AdvancedRateLimitEvent,
  ): Promise<void> {
    try {
      if (!event.ip) {
        throw new Error("IP cannot be null or empty");
  ...
  private async checkForAdvancedSuspiciousActivity(
    event: AdvancedRateLimitEvent,
  ): Promise<void> {
    try {
      if (!this.historyRepository) {
  ...
  public async getRateLimitMetrics(
    ...
    try {
      ...
      if (merchantId) {
        queryBuilder.andWhere("history.merchantId = :merchantId", {
          merchantId,
        });
      }
      if (userId) {
      ...
      return {
        ...
        throttledRequests,
        throttleRate:
          totalRequests > 0 ? (throttledRequests / totalRequests) * 100 : 0,
        burstRequests,
        burstRate:
          totalRequests > 0 ? (burstRequests / totalRequests) * 100 : 0,
        endpointStats,
        roleStats,
  ...
  public async getUserRateLimitHistory(
    userId: string,
    limit = 100,
  ...
  private async getTopThrottledIPs(
    startTime: Date,
    limit: number,
  ): Promise<TopThrottledIP[]> {
    try {
      if (!this.historyRepository) {
        return [];
      }
  ...
  private async getTopThrottledUsers(
    startTime: Date,
    limit: number,
  ): Promise<TopThrottledUser[]> {
    try {
      if (!this.historyRepository) {
        return [];
      }
  ...
  public async getRealTimeStatus(): Promise<RealTimeStatus> {
    try {
      ...
      const throttledRequests = recentActivity.filter(
        (r) => r.wasThrottled,
      ).length;
