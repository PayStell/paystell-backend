...
class RateLimitMonitoringService {
  ...
  private checkForSuspiciousActivity(event: RateLimitEvent): void {
    ...
    const recentIpEvents = ipEvents.filter(
      (e) => now - e.timestamp.getTime() <= this.suspiciousCriteria.timeWindowMs,
    );
    ...
    try {
      ...
      const throttledCount = await this.historyRepository.count({
        where: {
          ip: event.ip,
          wasThrottled: true,
          timestamp: { $gte: fiveMinutesAgo } as any,
        },
      });
  ...
  private determineMerchantType(merchant: any): string {
  ...
  private async getTopThrottledIPs(startTime: Date, limit: number): Promise<any[]> {
    try {
      if (!this.historyRepository) {
        return [];
      }
      ...
      return results.map(r => ({
        ip: r.ip,
        throttledCount: parseInt(r.count, 10)
      }));
    } catch (error) {
  ...
  private async getTopThrottledUsers(startTime: Date, limit: number): Promise<any[]> {
    try {
      if (!this.historyRepository) {
        return [];
      }
  ...
  public async getRealTimeStatus(): Promise<any> {
    try {
      const now = new Date();
