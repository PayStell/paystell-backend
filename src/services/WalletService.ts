...
import { Transaction, TransactionStatus, TransactionType, PaymentMethod } from "../entities/Transaction"
...
export class WalletService {
  ...
  async getWalletBalances(walletId: string): Promise<any[]> {
    ...
    try {
      ...
      console.warn(`Failed to fetch live balances: ${error?.message || "Unknown error"}, using cached data`)
      const cachedBalances = await this.balanceRepository.find({ where: { walletId } })
  ...
  async getTransactions(walletId: string, page = 1, limit = 20, sort: "asc" | "desc" = "desc") {
    const wallet = await this.walletRepository.findOne({ where: { id: walletId } })
    if (!wallet) {
      throw new Error("Wallet not found")
    }
  ...
  async updateWalletSettings(walletId: string, settings: any): Promise<Wallet> {
    const wallet = await this.walletRepository.findOne({ where: { id: walletId } })
    if (!wallet) {
      throw new Error("Wallet not found")
    }
  ...
  async getWalletInfo(walletId: string) {
    ...
    try {
      const accountInfo = await this.stellarService.getAccountInfo(wallet.publicKey)
      const accountExists = await this.stellarService.checkAccountExists(wallet.publicKey)
      ...
      throw new Error(`Failed to get wallet info: ${error?.message || "Unknown error"}`)
    }
  }
  ...
  async estimateTransactionFee(operationCount = 1): Promise<string> {
    try {
      ...
      throw new Error(`Failed to estimate fee: ${error?.message || "Unknown error"}`)
    }
  }
  ...
  async syncTransactionsFromBlockchain(walletId: string) {
    ...
    try {
      const stellarTransactions = await this.stellarService.getAccountTransactions(wallet.publicKey, undefined, 50)
      ...
      for (const stellarTx of stellarTransactions) {
        ...
        if (!existingTx) {
          const transaction = this.transactionRepository.create({
            hash: stellarTx.hash,
            type: TransactionType.PAYMENT,
            status: stellarTx.successful ? TransactionStatus.SUCCESS : TransactionStatus.FAILED,
