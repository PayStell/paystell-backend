...
export class ReferralService {
  ...
  private async createRewards(referral: Referral): Promise<void> {
    ...
    const totalRewardAmount =
      Number.parseFloat(activeProgram.referrerReward) +
      Number.parseFloat(activeProgram.refereeReward);
    const newUsedBudget =
      Number.parseFloat(activeProgram.usedBudget) + totalRewardAmount;

    if (
      activeProgram.totalBudget &&
      newUsedBudget > Number.parseFloat(activeProgram.totalBudget)
    ) {
